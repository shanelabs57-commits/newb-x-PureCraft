...
vec4 renderClouds(
  vec2 p,float t,float rain,vec3 horizonCol,vec3 zenithCol,
...
vec4 renderAurora(vec3 p,float t,float rain,float dayFactor) {
  ...
  return vec4(
    NL_AURORA*mix(NL_AURORA_COL1,NL_AURORA_COL2,d1),
    1.0
  )*d2*mask;
}
#endif
...
vec4 nlCloudAuroraReflection(
  nl_skycolor skycol,nl_environment env,vec3 viewDir,vec3 wPos,vec3 CAMERA_POS,highp float t
) {
  ...
  vec4 aurora=renderAurora(
    ...
    t,
    env.rainFactor,
    smoothstep(0.2,-0.2,env.dayFactor)
  );
  ...
  clouds.a*=fade;
  refl=vec4(
    mix(refl.rgb,clouds.rgb,clouds.a),
    min(refl.a+clouds.a,1.0)
  );
#endif
